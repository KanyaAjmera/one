    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
            ...
            // It's likely a Vite proxy error, HTML, or completely empty because the backend is dead
            if (!errorText || errorText.includes("proxy") || errorText.includes("HTML")) {
                errorText = "Backend API is unreachable. Please ensure your Python backend is running!";
            }
        }
        ...
        <div className="flex-1 flex flex-col items-center justify-start min-h-[400px]">
                ...
                <div className="flex-1 flex flex-col gap-6">
                    <div>
                        ...
                        <p className="text-white/60">
                            Describe your perfect profile picture and select a style. The AI enhancer will optimize your prompt automatically!
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-semibold text-white/80 uppercase tracking-wider">Style</label>
                            <select 
                                id="style"
                                value={style}
                            ...
                            >
                                <option className="bg-slate-900" value="anime">🌸 Anime</option>
                                <option className="bg-slate-900" value="realistic">📸 Realistic</option>
                                <option className="bg-slate-900" value="3d">🧊 3D Render</option>
                            </select>
                        </div>
                        ...
                        >
                            ...
                            {loading ? "Generating... (First time may take ~30s)" : "Generate Avatar"}
                        </button>
            ...
            {history.length > 1 && (
                <div className="w-full max-w-4xl mt-8">
                    <h3 className="text-xl font-bold mb-4 opacity-80">Recent Avatars</h3>
                    <div className="flex gap-4 overflow-x-auto pb-4">
                        {history.map((url, i) => (
                            ...
                            >
                                <img src={url} alt={`History item ${i}`} className="w-full h-full object-cover" />
                            </div>
                        ))}
                    </div>
                </div>
