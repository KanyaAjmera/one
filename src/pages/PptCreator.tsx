  ...
  const handleGeneratePptAI = async () => {
    ...
    try {
      ...
      if (res.ok && data.slides) {
        ...
        const fileName = (data.presentationTitle || topic).trim() ? `${(data.presentationTitle || topic).replace(/\s+/g, '_')}.pptx` : 'Presentation.pptx';

        data.slides.forEach((slideData: any) => {
          const slide = pres.addSlide();
          const isDarkBackground = slideData.type === 'Title Slide' || slideData.type === 'Conclusion Slide';
          slide.background = { fill: isDarkBackground ? colors.primary : colors.background };
          ...
          if (slideData.type === 'Title Slide') {
            slide.addShape(pres.ShapeType.rect, { x: 0.5, y: 3.5, w: 12.33, h: 0.05, fill: { color: colors.accent } });
            
            slide.addText(slideData.title, {
              x: 1.0, y: 1.8, w: 11.33, h: 1.5,
              fontSize: 44, bold: true, align: 'center', color: 'FFFFFF', fontFace: 'Trebuchet MS'
            });
            if (slideData.subtitle) {
              slide.addText(slideData.subtitle, {
                x: 1.0, y: 3.8, w: 11.33, h: 0.8,
                fontSize: 22, align: 'center', color: colors.accent, fontFace: 'Calibri'
              });
            }
          }
            ...
            slide.addText(slideData.title, {
              ...
              fontSize: 40, bold: true, align: 'center', color: 'FFFFFF', fontFace: 'Trebuchet MS'
            });
            if (slideData.content && slideData.content.length > 0) {
              const bullets = slideData.content.map((b: string) => ({ text: b, options: { bullet: true, breakLine: true } }));
              slide.addText(bullets, {
                x: 2.0, y: 3.8, w: 9.33, h: 2.5,
                fontSize: 18, align: 'center', color: 'FFFFFF', fontFace: 'Calibri'
              });
            }
          }
          else {
            slide.addText(slideData.title, {
              ...
              fontSize: 28, bold: true, color: colors.primary, fontFace: 'Trebuchet MS'
            });
            
            if (slideData.subtitle) {
              slide.addText(slideData.subtitle.toUpperCase(), {
                x: 0.8, y: 0.25, w: 11.73, h: 0.3,
                fontSize: 11, bold: true, color: colors.accent, fontFace: 'Calibri'
              });
            }
            
            slide.addShape(pres.ShapeType.line, {
              x: 0.8, y: 1.3, w: 11.73, h: 0.01,
            ...
            if (layoutType === 'Two Column') {
              ...
              const leftBullets = slideData.content.slice(0, halfIndex).map((b: string) => ({ text: b, options: { bullet: true, breakLine: true } }));
              const rightBullets = slideData.content.slice(halfIndex).map((b: string) => ({ text: b, options: { bullet: true, breakLine: true } }));
              
              slide.addText(leftBullets, {
                x: 0.8, y: 1.8, w: 5.5, h: 4.8,
                fontSize: 15, color: colors.text, fontFace: 'Calibri', valign: 'top', lineSpacing: 24
              });
              slide.addText(rightBullets, {
                x: 7.0, y: 1.8, w: 5.5, h: 4.8,
                fontSize: 15, color: colors.text, fontFace: 'Calibri', valign: 'top', lineSpacing: 24
              });
            }
            else if (layoutType === 'Image Left') {
              slide.addShape(pres.ShapeType.rect, {
                x: 0.8, y: 1.8, w: 5.2, h: 4.8,
                fill: { color: colors.background === 'F9F9F9' ? 'EDEDED' : 'EAEAEA' },
              ...
              slide.addText(`Visual Concept:\n\n"${slideData.imagePrompt || 'Recommended visual representing ' + slideData.title}"`, {
                x: 1.0, y: 2.2, w: 4.8, h: 4.0,
                fontSize: 13, color: colors.text, fontFace: 'Calibri', italic: true, align: 'center', valign: 'middle'
              });
              ...
              if (slideData.content && slideData.content.length > 0) {
                const bullets = slideData.content.map((b: string) => ({ text: b, options: { bullet: true, breakLine: true } }));
                slide.addText(bullets, {
                  x: 0.8, y: 1.8, w: 5.9, h: 4.8,
                  fontSize: 15, color: colors.text, fontFace: 'Calibri', valign: 'top', lineSpacing: 24
                });
              }
              
              slide.addShape(pres.ShapeType.rect, {
                x: 7.3, y: 1.8, w: 5.2, h: 4.8,
                fill: { color: colors.background === 'F9F9F9' ? 'EDEDED' : 'EAEAEA' },
                line: { color: colors.secondary, width: 1.5 }
              });
              slide.addText(`Visual Concept:\n\n"${slideData.imagePrompt || 'Recommended visual representing ' + slideData.title}"`, {
                x: 7.5, y: 2.2, w: 4.8, h: 4.0,
                fontSize: 13, color: colors.text, fontFace: 'Calibri', italic: true, align: 'center', valign: 'middle'
              });
            }
              ...
              slide.addText(rightBullets, {
                x: 7.2, y: 2.0, w: 5.1, h: 4.4,
                fontSize: 15, color: colors.text, fontFace: 'Calibri', valign: 'top', lineSpacing: 24
              });
            }
            else if (layoutType === 'Timeline') {
              slide.addShape(pres.ShapeType.line, {
                x: 0.8, y: 4.0, w: 11.73, h: 0,
              ...
              slideData.content.forEach((stepText: string, idx: number) => {
                ...
                slide.addShape(pres.ShapeType.ellipse, {
                  x: 0.8 + (idx * stepW) + (stepW/2) - 0.125, y: 3.875, w: 0.25, h: 0.25,
                ...
                slide.addShape(pres.ShapeType.rect, {
                  x: stepX, y: stepY, w: stepBoxW, h: stepH,
                  fill: { color: 'FFFFFF' },
                  line: { color: colors.secondary, width: 1 }
                });
                
                slide.addText(`Step ${idx + 1}`, {
                  x: stepX + 0.05, y: stepY + 0.1, w: stepBoxW - 0.1, h: 0.3,
                  fontSize: 12, bold: true, color: colors.primary, fontFace: 'Trebuchet MS', align: 'center'
                });
                
                slide.addText(stepText, {
                  x: stepX + 0.05, y: stepY + 0.45, w: stepBoxW - 0.1, h: stepH - 0.55,
                  fontSize: 10, color: colors.text, fontFace: 'Calibri', align: 'center', valign: 'top'
                });
              });
            }
              ...
              try {
                ...
                slide.addText(`[Interactive ${chartType.toUpperCase()} Chart]\n\nVisual representation of data.`, {
                  x: chartX + 0.2, y: chartY + 2.0, w: chartW - 0.4, h: 1.0,
                  fontSize: 14, color: colors.text, fontFace: 'Calibri', align: 'center', valign: 'middle'
                });
              }
            }
            else {
              if (slideData.content && slideData.content.length > 0) {
                const bullets = slideData.content.map((b: string) => ({ text: b, options: { bullet: true, breakLine: true } }));
                slide.addText(bullets, {
                  x: 0.8, y: 1.8, w: 11.73, h: 4.8,
                  fontSize: 16, color: colors.text, fontFace: 'Calibri', valign: 'top', lineSpacing: 28
                });
              }
            }
          }
