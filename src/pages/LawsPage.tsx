...
export default function LawsPage() {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.map(cat => ({
        ...
        laws: cat.laws.filter(law => {
          const searchString = `${law.title} ${law.description} ${law.section} ${law.law} ${law.keywords.join(' ')}`.toLowerCase();
  ...
  return (
    <div className={`flex flex-col h-screen w-screen relative overflow-hidden transition-colors duration-1000 ${isLightMode ? 'bg-white/0 text-black' : 'bg-black/0 text-white'}`}>
                ...
                <div key={categoryData.category} className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {categoryData.laws.map((law, idx) => (
                      ...
                      >
                        <h3 className="text-xl font-bold capitalize mb-1">{law.title}</h3>
                        <p className={`text-sm font-medium ${isLightMode ? 'text-gray-500' : 'text-gray-400'}`}>
                          {law.law} <span className="opacity-50 mx-1">•</span> Sec {law.section}
                        </p>
                        
                        <div className="mt-3 flex flex-wrap gap-2">
                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-sm ${law.bailable ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'}`}>
                                {law.bailable ? "Bailable" : "Non-Bailable"}
                            </span>
                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-sm ${law.cognizable ? 'bg-orange-500/20 text-orange-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-500/20 flex-1">
                          ...
                          {law.elements && law.elements.length > 0 && (
                            <div className="mt-3">
                              <p className={`text-xs font-semibold uppercase ${isLightMode ? 'text-gray-400' : 'text-gray-500'} mb-1`}>Elements</p>
                              <ul className="list-disc list-inside text-xs space-y-0.5 opacity-80">
                                {law.elements.map((el, i) => <li key={i}>{el}</li>)}
                              </ul>
                            </div>
                          )}
                          ...
                          <div className="mt-4 inline-flex flex-col px-3 py-2 rounded-lg bg-emerald-500/10 text-emerald-500 text-xs font-medium border border-emerald-500/20 w-full">
                            <span className="font-bold uppercase tracking-wide mb-1">Punishment</span>
                            <span>{law.punishment.type.join(', ')}</span>
                            {law.punishment.duration && <span>Duration: {law.punishment.duration}</span>}
                            {law.punishment.fine && <span>Fine: {law.punishment.fine}</span>}
                            {law.punishment.note && <span>Note: {law.punishment.note}</span>}
                          </div>
                        </div>
                      </div>
                    ))}
